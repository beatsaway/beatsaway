...
class BeatSegment {
    ...
    initializeSoundParams() {
        ...
        // First try to load default preset if available
        if (window.soundPresets && window.soundPresets[this.drumType]) {
            const defaultPreset = window.soundPresets[this.drumType][0];
            if (defaultPreset && defaultPreset.params) {
                Object.assign(soundParams, defaultPreset.params);
            }
        }

        // Then try to get parameters from the drum module if available
        ...
        if (drumModuleClass && drumModuleClass.parameterNames) {
            ...
            for (const [paramName, paramConfig] of Object.entries(parameters)) {
                // Only set parameter if it's not already set from presets
                if (!(paramName in soundParams)) {
